...
const Navbar = ({ hideEnquireButton = false }) => {
  ...
  const handleLinkClick = (path, setDropdownState) => {
    ...
    // Handle paths with query parameters (e.g., /find-space?city=Salem)
    if (path.includes('?')) {
    ...
    if (path.includes('#')) {
      ...
      if (window.location.pathname !== targetRoute) {
        ...
        setTimeout(() => {
          ...
          if (element) {
            const yOffset = -80; // Offset for navbar header
      ...
      <nav className={`bg-white shadow-sm fixed w-full top-0 z-50 transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
        <div className="container mx-auto px-0 py-4">
          <div className="flex items-center justify-between">
            {/* Logo on the left */}
            <div className="flex items-center space-x-2 pl-3 md:pl-6 cursor-pointer" onClick={() => navigate('/')}>
              ...
              <div className="flex items-center justify-center flex-1 mx-4">
                ...
                >
                  <button
                    className="nav-button text-slate-800 text-base font-semibold hover:text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-100 flex items-center"
                    onClick={() => {
                      navigate('/');
                      window.scrollTo(0, 0);
                      setIsWhyTidelDropdownOpen(false);
                ...
                >
                  <button
                    className="nav-button text-slate-800 text-base font-semibold hover:text-blue-600 transition-all duration-300 px-3 py-2 rounded-lg hover:bg-slate-100 flex items-center"
                    onClick={() => {
                      navigate('/about-us');
                      window.scrollTo(0, 0);
                      setIsAboutUsDropdownOpen(false);
                ...
                <button
                  ...
                  className="nav-button text-slate-800 text-base font-semibold hover:text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-100 flex items-center ml-2"
                >
                  Tenders
                </button>
            ...
            {!hideEnquireButton && (
              <div className="opacity-0 translate-y-3 animate-fade-in-up pr-4 md:pr-6 relative z-10" style={{ animationDelay: '1000ms' }}>
                <button
